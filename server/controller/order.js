  ...
  db.query('select balance from user_info where userId ='+userId,function (error,results,fields) {
   var balance = results[0].balance;
    ...
    db.query('UPDATE user_info SET balance = balance -'+totalMoney+' where userId ='+userId,function (error,results,fields){
           db.query('UPDATE order_list_detail set goodsStatus = 1',function (error,results,fields){
            callback(true);
          })
    });
...
var delivery = function (goodsId,callback){
    db.query('update goods_list set status = 3 where goodsId = '+goodsId,function (error,results,fields){
      callback();
    })
    db.query('update order_list_detail set goodsStatus = 3 where goodsId = '+goodsId)
}

//收货
var takeGoods = function (goodsId,callback){ 
   db.query('update goods_list set status = 4 where goodsId = '+goodsId,function(){
      callback();
   })
